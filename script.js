// ===== Configuration =====
const CONFIG = {
  WEATHER_API_KEY: "82ef7eb8c710a4d63f28712218fd2b3e", // Get from openweathermap.org
...
function showRecommendations(weather, trend, city) {
  ...
  // Display as cards with search prompts
...
function getLocalTrend(city) {
  const trends = {
    ...
    "London": ["Pub culture", "Royal sights", "Underground art"],
    "Tokyo": ["Cherry blossoms", "Robot restaurants", "Temple visits"]
  };
  return trends[city]?.[Math.floor(Math.random() * trends[city].length)] || "local culture";
}
